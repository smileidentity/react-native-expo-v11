const { withAndroidStyles, withInfoPlist, withAppBuildGradle, withProjectBuildGradle } = require('@expo/config-plugins');

const DEFAULT_ANDROID_SDK_VERSION = 36;

const withSmileID = (config, props = {}) => {
  const {
    smileConfigPath,
  ...
  if (smileConfigPath) {
    config = withAppBuildGradle(config, (newConfig) => {
      if (newConfig.modResults.language === 'groovy') {
        ...
        if (!buildGrad.includes('smile_config.json')) {
          newConfig.modResults.contents = buildGradle.replace(
            /android\s*\{/,
